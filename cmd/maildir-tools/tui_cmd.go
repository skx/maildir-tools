//
// TODO
//  1. [x] Setup a stack of past modes.
//
//  2. [x] Bind global `q` to go back to the previous state, using that stack.
//
//  3. [x] Bind `Q` to quit, globally.
//
//  4. [x] When changing to a folder with only one message the onChange
//         handler is not called.  Set selection to first item manually.
//
//  5. [x] Add "/" to search forward.  Wrapping around appropriately.
//         This should work in all modes.
//
//  6. [x] Add more default keybindings to match lumail:
//         Vi emulation: j:Down + k:Up
//         Steve: <:Home >:end *:end
//
//         NOTE: These should be trivial to add to config-file if we had one.
//
//  7. [x] Add prompt function for listbox searching
//         // Same as #5.
//
//  8. [ ] Update helper  (What does that mean?)
//
//  9. [ ] Commit.
//
// 10. [ ] Cache maildir list?  Or at least make the display faster by default
//
// 11. [ ] Add help-text for real.
//         Global-help text is good.
//         Ideally append per-mode help text too.
//
// 12. [x] When changing mode back we reset the offset of the selected item
//         in the current/old mode.  We should save and restore that.
//         TODO: This could be improved as we store across changes.
//
// 13. [ ] Need to enable line-wrapping for list-items.
//         (This requires UI-toolkit support.)
//
// 14. [ ] The mail/help-views look ugly with the message/line-highlighting
//         (Either change to a different UI-widget, or patch.)
//
// 15. [x] BUG: Binding global keybindings break the text-search
//         e.g. "j -> down" means I can't do a search for "jenny".
//         Hacky Fix: Check for list-view being focused before sending.
//
// Steve
// --
//
